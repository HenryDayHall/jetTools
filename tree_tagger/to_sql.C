...
void PrintJet(Jet &jet, int verbosity, string &out) {
    std::stringstream stream;
    if(verbosity < 0){
         stream <<  jet.PT << ", ";
         stream <<  jet.Eta << ", ";
         ...
         stream << "Phi = " <<  jet.Phi;
         stream << "T = " <<  jet.T;
         stream << "Mass = " <<  jet.Mass;
         stream << "DeltaEta = " <<  jet.DeltaEta;
         stream << "DeltaPhi = " <<  jet.DeltaPhi;
    } else {
         stream << "jet transverse momentum" << jet.PT;
         stream << "jet pseudorapidity" << jet.Eta;
         stream << "jet azimuthal angle" << jet.Phi;
         stream << "particle arrival time of flight" << jet.T;
         stream << "jet invariant mass" << jet.Mass;
         stream << "jet radius in pseudorapidity" << jet.DeltaEta;
         stream << "jet radius in azimuthal angle" << jet.DeltaPhi;
    }
    out = stream.str();
...
sqlite3 * CreateDatabase(const char *rootFile){
    ...
    sql = "CREATE TABLE Events("
          "ID INTEGER PRIMARY KEY NOT NULL," // Primary key for the event
          "Number INTEGER," // event number in root file
          "FilePath STRING);"; // the full file path to the original root file
    ...
    /* Create the table. Create SQL statement */
    sql = "CREATE TABLE Jets("
          "ID INTEGER PRIMARY KEY NOT NULL," // Primary key for the jet
          "PT REAL," // jet transverse momentum
          "Eta REAL," // jet pseudorapidity
          "Phi REAL," // jet azimuthal angle
          "T REAL," //particle arrival time of flight
          "Mass REAL," // jet invariant mass
          "DeltaEta REAL,"  // jet radius in pseudorapidity
          "DeltaPhi REAL,"  // jet radius in azimuthal angle
...
void AddEvent(Event &event, int SQLkey, sqlite3 * db){
    //Adds the particle, but not the relations to other particles
    char *zErrMsg = 0;
    int rc;
    const char *sql;
    const char * filePath = sqlite3_db_filename(db, "main");

    /* Insert the particle, create a statment*/
    std::stringstream glue;
    glue << "INSERT INTO Events(";
    glue << "ID, Number, FilePath)";
    glue << " VALUES (" << SQLkey << ", " << event.Number << ", \'";
    glue << filePath << "\');";
    const string tmp = glue.str();
    sql = tmp.c_str();
...
void AddJet(Jet &jet, int SQLkey, int eventKey, sqlite3 * db){
    //Adds the jet, but not the relations to other particles
    char *zErrMsg = 0;
    int rc;
    const char *sql;
    ...
    /* Insert the particle, create a statment*/
    std::stringstream glue;
    glue << "INSERT INTO Jets(";
    glue << "ID, PT, Eta, Phi, T, Mass, DeltaEta, DeltaPhi, Event)";
    glue << " VALUES (" << SQLkey << ", ";
    string jet_print = "";
    PrintJet(jet, -1, jet_print);
    glue << jet_print << ", " << eventKey << ");";
    const string tmp = glue.str();
    sql = tmp.c_str();
...
void AddParticle(GenParticle &particle, int SQLkey, int eventKey, sqlite3 * db){
    //Adds the particle, but not the relations to other particles
    char *zErrMsg = 0;
    int rc;
    const char *sql;
...
void AddTrack(Track &track, std::map<string, int> &particle_to_SQLkey, int SQLkey, int eventKey, sqlite3 * db){
    char *zErrMsg = 0;
    int rc;
    const char *sql;
    ...
    glue << track_print << ", " << particle_foreignKey << ", " << eventKey << ");";
    const string tmp = glue.str();
    sql = tmp.c_str();
...
void AddTower(Tower &tower, std::map<string, int> &particle_to_SQLkey, int SQLkey, int eventKey, sqlite3 * db){
    char *zErrMsg = 0;
    int rc;
    const char *sql;
...
void MakeTable(ExRootTreeReader *treeReader, sqlite3 *db) {
    ...
    Long64_t eventKey = 0, trackKey = 0, towerKey = 0, particleKey = 0, jetKey = 0;
    ...
    //for(eventKey = 0; eventKey < allEntries; ++eventKey) {
    for(eventKey = 0; eventKey < 2; ++eventKey) {
        // Load selected branches with data from specified event
        treeReader->ReadEntry(eventKey);
        // put the entry in the table
        event = *((Event *)branchEvent->At(0));
        AddEvent(event, eventKey, db);

        cout << "--	New event -- " << endl;

        cout << "Adding particles \n";
        //Loop over particle in the event
        // A new event has a fresh particle map
        particle_to_SQLkey.clear();
        //for(i=0; i < 2; i++){
        for(i=0; i < branchParticle->GetEntriesFast(); i++){
            particle = *((GenParticle *)branchParticle->At(i));
            AddParticle(particle, particleKey, eventKey, db);
            PrintParticle(particle, -1, particle_print);
            if(particle_to_SQLkey.find(particle_print) != particle_to_SQLkey.end()){
                cout << "ERROR! particle "<< particle_print << " found twice!\n";
            }
            particle_to_SQLkey[particle_print] = particleKey;
            particleKey++;
        }
        cout << "Particles added. \n";
        if(false){ // normally dont bother with heritage
            // Not working because https://cp3.irmp.ucl.ac.be/projects/delphes/ticket/1138
            // Basically there are often more that 2 daughters/mothers
            // its fine because the hepmc file has full heritage
            cout << "Looping again to assign heratage\n";
            for(i=0; i < branchParticle->GetEntriesFast(); i++){
                particle = *((GenParticle *)branchParticle->At(i));
                PrintParticle(particle, -1, particle_print);
        ...
        if(false){ // normally don't need the check
        ...
        for(int i=0; i< branchTower->GetEntriesFast(); i++){
            ...
            AddTower(tower, particle_to_SQLkey, towerKey, eventKey, db);
            towerKey++;
        }

        // Loop over all jets in event
        // we can fill the jet table and add references from the tracks/towers
        cout << "Adding jets\n";
        for(i = 0; i < branchJet->GetEntriesFast(); ++i) {
            jet = *((Jet*)branchJet->At(i));
            AddJet(jet, jetKey, eventKey, db);
            ...
            for(j = 0; j < jet.Constituents.GetEntriesFast(); ++j)
            {
                ...
                if(object->IsA() == GenParticle::Class()) {
                    cout << "Why is there a GenParticle in the jet?" << endl;
                    //particle = *((GenParticle*)object);
                    //PrintParticle(particle, -1, particle_print);
                    //int particleKey = particle_to_SQLkey[particle_print];
                    //UpdateRecord(db, "GenParticles", particleKey, "Jet", jetKey);
...
void MakeCheckFile(ExRootTreeReader *treeReader, const char* rootFile) {
    ...
    // Loop over all events
    //for(eventKey = 0; eventKey < allEntries; ++eventKey) {
    for(eventKey = 0; eventKey < 1; ++eventKey) {
        // Load selected branches with data from specified event
        treeReader->ReadEntry(eventKey);
...
void to_sql()
{
    const char *inputFile = "~/lazy/h1bBatch2.root";
