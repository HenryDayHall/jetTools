...
c..
      ...
      !for the complex MSSM
      double precision alphatree, SAtree
      double complex Hmixtree(3,3)
      double complex ZHiggs(3,3)
...
!     setting read-in-flags to false
      ...
      lslhaalpha     = .false.
      lexpandc1o1    = .true.
      feynhiggspar(-1) = 0 !default running with direct link
      thdmcpar(-1) = 0 !default running with direct link
      
      !read in of the main parameters for SUSHI
      blocks = ' '
      blocks(1) = 'SUSHI'
      call readblocks(12,blocks,nkeys)
...
c..   translate from old notation for Higgs type to NMSSM-style:
     ...
     &           "Higgs masses, mu and stop, sbottom masses/angles.")
      ...
      !looking for block FlexibleSUSY, let FS+H run and replace input file   
         bltype = 'FLEXIBLESUSY'
         call readslha(12,bltype,blockflag)
         if(blockflag.eq.1) then
            call printinfosushi(6,
     &           "SusHi was called with the SLHA-block 'FLEXIBLESUSY':")
            call printinfosushi(6,
     &           "Thus FlexibleSUSY+Himalaya ")
            call printinfosushi(6,
     &           "is used for the calculation of SUSY")
            call printinfosushi(6,
     &           "Higgs masses, mu and stop, sbottom masses/angles.")
            fshflag=.true.
            
      !if FS+H is used, take the inputfile, give it to FlexibleSUSY and 
      !use the result as On-Shell input for SusHi
      !(FSH-output contains Block STOPMIX, which is here used to 
      !identfy OS input)

            close(12)
            open(unit=12,file=jfnameact)
            call runcmdfsh(jfilein,jfnameact)
  
         endif 

      !if SusHi gets an extern file (e.g. output from Feynhiggs), concatenatefiles
      !it with sushi input and use resulting file as further input
         bltype = 'EXTFILE'
         call readslha(12,bltype,blockflag)
         if (blockflag.eq.1) then
           blocks = ' '
           ...
           close(12)
           open(unit=12,file=jfnameact)
           call concatenatefiles(jfilein,extfilepar(1),jfnameact)
         endif
...
!         !onshellflag is always false at this point.
!         if ((.not.onshellflag)
!     &.and.(.not.fhcmdflag).and.(.not.lextfilepar(1))
!     &.and.(.not.fshflag)) then
!        write(*,*) "Inputfile does neither contain Block FEYNHIGGS,"
!        write(*,*) "FLEXIBLESUSY or an adequate MSSM spectrum."
!        write(*,*) "Please fix this or give an extern file with masses"
!        write(*,*) "and mixing angles under Block EXTFILE 1."
!         stop
!         endif    
     ...
     &      "scheme using on-shell parameter input.")
            ...
            p2approx = 4
            looplevel = 2
            loglevel = 3 !changed prescription May 2016 from 0 to 3
            runningMT = 1
            botResum = 1
            tlCplxApprox = 0
            ZHiggsapply = 0
            ...
            if (lfhflags(11)) ZHiggsapply = fhflagspar(11)
         end if
...
!            call readblocks(12,blocks,nkeys)
     ...
     &"Complex MSSM needs a link to the FeynHiggs library.")
         else
            blocks(1) = 'FEYNHIGGS'
            blocks(2) = 'FEYNHIGGSFLAGS'
            call readblocks(12,blocks,nkeys)
            ...
            fieldren = 0
            tanbren = 0
            higgsmix = 3 !2 in real MSSM
            p2approx = 4
            looplevel = 2
            loglevel = 3 !changed prescription May 2016 from 0 to 3
            runningMT = 1
            botResum = 1
            tlCplxApprox = 3 !0 in real MSSM
            ZHiggsapply = 1
            if (lfhflags(1)) mssmpart = fhflagspar(1)
            if (lfhflags(2)) fieldren = fhflagspar(2)
            if (lfhflags(3)) tanbren = fhflagspar(3)
...
!          runningMT = 0
     ...
     &     ,'SMINPUTS(6): top mass undefined. ')
      ...
      !if Feynhiggs used in CMD-mode get STOPMIX etc. from feynhiggs output-file: 
      if ((feynhiggspar(-1).eq.1).and.(model.eq.1)) then
      
        close(12)
        open(unit=12,file=jfnameact)
        call runcmdfeynhiggs(jfilein,jfnameact)
     ...
     &      "scheme using on-shell parameter input.")
      ...
      !if 2HDMC used in CMD-mode get Higgs-masses from 2HDMC output-file      
      if ((thdmcpar(-1).eq.1).and.(model.eq.2)) then
      
        close(12)
        open(unit=12,file=jfnameact)
        call runcmdthdmc(jfilein,jfnameact)
      ...
      !if Feynhiggs used in CMD-mode get STOPMIX etc. from feynhiggs output-file: 
      if ((feynhiggspar(-1).eq.1).and.(model.eq.10)) then
       call printerrorsushi(1,6,
     &"cmd line mode of SusHi not (yet) compatible with CP violation.")
      end if
...
!           end do
     ...
     &     lnmhmix(count1,count2),'NMHMIX partially undefined. ')
         ...
         do count1 = 1,3
         do count2 = 1,3
         CHmx(count1,count2) = dcmplx(Hmx(count1,count2),0.d0)
         end do
         end do
     ...
     &              ,'pseudoscalar mass undefined. ')
            ...
            else if (lextpar(27)) then
            MHp = mapreal8(extpar(27),lextpar(27),'MHp undefined. ')
            else
            call printdie("FeynHiggs needs m_A or m_H+ as input.")
            end if
     ...
     &'M1SD for FH undefined. ')
      ...
      !---CMSSM
         ...
         !SUSY parameters from extpar or external masses
         if (.not.fhflag) then
            val(1) = mapreal8(dabs(extpar(3)),lextpar(3)
     &     ,'gluino mass undefined. ')
            val(2) = mapreal8(imextpar(3),limextpar(3)
     &     ,'gluino mass undefined. ')
            CMgl = DCMPLX(val(1),val(2))
            Mgl = Abs(CMgl)
            val(1) =mapreal8(extpar(23),lextpar(23),'mu undefined. ')
            val(2) =mapreal8(imextpar(23),limextpar(23),'mu undefined.')
            CmuSUSY = DCMPLX(val(1),val(2))
            muSUSY = Abs(CmuSUSY)
            !or calculate them internally later in renscheme.f
            !original option of SusHi
            M3SQ = mapreal8(extpar(43),lextpar(43),'M_SQ undefined. ')
            M3SU = mapreal8(extpar(46),lextpar(46),'M_SU undefined. ')
            M3SD = mapreal8(extpar(49),lextpar(49),'M_SD undefined. ')
            val(1) =mapreal8(extpar(11),lextpar(11),'At undefined. ')
            val(2) =mapreal8(imextpar(11),limextpar(11),'At undefined.')
            CAt = DCMPLX(val(1),val(2))
            val(1) =mapreal8(extpar(12),lextpar(12),'Ab undefined. ')
            val(2) =mapreal8(imextpar(12),limextpar(12),'Ab undefined.')
            CAb = DCMPLX(val(1),val(2))
            Mh = mapreal8(smassbo(25+10*(Sind-1))
     &       ,lsmassbo(25+10*(Sind-1)),'Higgs mass undefined. ')
            alphatree=mapreal8(slhaalpha,lslhaalpha,'alpha undefined. ')
            ZHiggs = (0.d0,0.d0)
            do count1 = 1,3
            do count2 = 1,3
            !for the time being only real input
            ZHiggs(count1,count2) = mapreal8(nmhmix(count1,count2),
     &       lnmhmix(count1,count2),'NMHMIX partially undefined. ')
            end do
            end do
            Hmx = 0.d0
            ...
            !building the Higgs mixing matrix from the Z factors and
            !the tree-level Higgs mixing in the CP-even sector
            Hmixtree = DCMPLX(0.d0,0.d0)
            SAtree = dsin(alphatree)
            Hmixtree = DCMPLX(0.d0,0.d0)
            Hmixtree(1,1) = DCMPLX(-SAtree,0.d0)
            Hmixtree(1,2) = DCMPLX(dsqrt(1.d0-SAtree*SAtree),0.d0)
            Hmixtree(2,1) = DCMPLX(dsqrt(1.d0-SAtree*SAtree),0.d0)
            Hmixtree(2,2) = DCMPLX(SAtree,0.d0)
            Hmixtree(3,3) = DCMPLX(1.d0,0.d0)
            CHmx = Matmul(ZHiggs,Hmixtree)
            ...
            val(2) =mapreal8(imextpar(12),limextpar(12),'Ab undefined.')
            CAb = DCMPLX(val(1),val(2))
            val(1) = mapreal8(extpar(3),lextpar(3)
     &     ,'gluino mass undefined. ')
            val(2) = mapreal8(imextpar(3),limextpar(3)
     &     ,'gluino mass undefined. ')
            CMgl = DCMPLX(val(1),val(2))
            ...
            M_3= CMgl
!            MA0 = mapreal8(extpar(26),lextpar(26),'MA0 undefined. ')
            MHp = mapreal8(extpar(27),lextpar(27),'MHp undefined. ') !CMSSM only accepts MHp
            M_1=dcmplx(mapreal8(feynhiggspar(1),lfeynhiggspar(1),
     &'M_1 undefined. '))
            M_2=dcmplx(mapreal8(feynhiggspar(2),lfeynhiggspar(2),
     &'M_2 undefined. '))
...
!---  2HDM
      if (model.eq.2) then
         if ((.not.thdmcflag).and.(thdmcpar(-1).eq.0)) then
    ...
    !  if thdmcpar(-1)=1, get Higgs-masses from 2HDMC via Command-line executed before    
    
         else if (thdmcpar(-1).eq.1) then
            twohdmver = mapreal8(thdmcpar(2),lthdmcpar(2),
     &           '2HDM version undefined. ')
            tanb = mapreal8(thdmcpar(3),lthdmcpar(3),'tanb undefined. ')
            alpha = mapreal8(slhaalpha,lslhaalpha,'alpha undefined. ')
            if (pseudo.eq.0) then
             if (Sind.eq.1) then
               Mh = mapreal8(smassbo(25),lsmassbo(25)
...
!     some settings for FeynHiggs if used
      if ((model.eq.1).or.((model.eq.0).and.fhflag)
     &.or.(model.eq.10)) then
         CKMlambda = -1
         CKMA      = -1
         CKMrhobar = -1
     ...
     &     'bottom yukawa-coupling undefined. ')
      ...
      if ((model.eq.1).or.(model.eq.3).or.(model.eq.10)) then
...
!  6   0.d0             # sb'
     ...
     &           ,'Block SUSHI, entry 6 out of range.')
         ...
         if ((ZHiggsapply.eq.1).and.(nnlostop)) then
            call printerrorsushi(1,6
     &       ,'ZHiggs not compatible with NNLO stop contributions.')
         end if
...
C-{{{ subroutine outputsushi
      ...
      subroutine outputsushi(ggchanA,qgchanA,qqchanA,
     & ggchanerrA,qgchanerrA,qqchanerrA,
     & SUbbhA,SUbbherrA,SUgghA,SUggherrA,
     & xsecA,xsecerrA,xeffA,xefferrA,
     & xeffvarA,gghXSvarA,
     & gb13A,gbh123A,gbh213A,gb23A,gt13A,
     & gth123A,gth213A,gt23A,gbA,gcA,gtA,elwA,
     & etaA,BRA)
      ...
      character effresstring2*34,bbhstring*3
      integer date_time(8)
      character realcl(3)*12,tmpstring*4,orderstring(4)*4,
     &     fhvers*20,hbvers*20,hsvers*20,thdmcvers*20
      !arrays relevant in case of CP violation
      double complex gb13A(2), gbh123A(2), gbh213A(2), gb23A(2)
      double complex gt13A(2), gth123A(2), gth213A(2), gt23A(2)
      double complex gbA(2), gcA(2), gtA(2)
      double precision ggchanA(0:2),qgchanA(0:2),qqchanA(0:2), elwA(0:2)
      double precision ggchanerrA(0:2),qgchanerrA(0:2),qqchanerrA(0:2)
      double precision SUbbhA(0:2,4),SUgghA(0:2,4,4,-1:5)
      double precision xsecA(0:2,2),xsecerrA(0:2,2)
      double precision xeffA(0:2), xefferrA(0:2)
      double precision xeffvarA(0:2,1:2),gghXSvarA(0:2,1:2)
      double precision SUbbherrA(0:2,4),SUggherrA(0:2,4,4,-1:5)
      double precision etaA(2,8),BRA(8)
...
c.. Henry's edits; we will read the output from stdout
      open(unit=13,file="/dev/stdout",status='old')
c..   open(unit=13,file=jfnameact,status='unknown')
...
c.. Henry's edits; prompt python to start looking for output
      write(13, *) '**output file starts here'
c.. Henry's edits; also cut the output down to the bits we really want
      out_verbose=.FALSE.
      if (out_verbose) then
     ...
     &    "#--------------------------------------------------#"
      ...
      effres = xeffA(0)
      effreserr = xefferrA(0)
      if (norderggh.eq.0) then
         effres = xsecA(0,1)
         effreserr = xsecerrA(0,1)
      endif
...
!xeffd,xeffu,murgghd,murgghu
     ...
     &    '+/- integ. error: ggh '//effresstring
          if (muranalytic) then
          write(13,102) 102,gghXSvarA(0,1),
     &    '- from muR variation: ggh '//effresstring
          write(13,102) 103,gghXSvarA(0,2),
     &    '+ from muR variation: ggh '//effresstring
          write(13,1030) '# within [',dexp(xeffvarA(0,1))/murggh,
     &  ',',dexp(xeffvarA(0,2))/murggh,']*SCALES(1)*MASSOUT(1).'
          end if
     ...
     &        "# Note: Fixed order p_T distribution (no resummation)!"
      ...
      if (citations(60).gt.0) then !interference factors, when compiled with FHINT, only H2,H3 implemented
      write(13,103) 'Block INTER # Factors XS*(1+INTER)*BR at LO'
      write(13,108) 21,15,etaA(1,1),'gg -> tau^+tau^-'
      write(13,108) 21,5, etaA(1,2),'gg -> bb~'
      write(13,108) 21,6, etaA(1,3),'gg -> tt~'
      write(13,108) 21,22,etaA(1,4),'gg -> gaga'
      write(13,108) 21,24,etaA(1,5),'gg -> WW'
      write(13,108) 21,23,etaA(1,6),'gg -> ZZ'
      write(13,108) 21,21,etaA(1,7),'gg -> gg'
      write(13,108) 21,25,etaA(1,8),'gg -> hh'
      write(13,108) 5,15,etaA(2,1),'bb -> tau^+tau^-'
      write(13,108) 5,5, etaA(2,2),'bb -> bb~'
      write(13,108) 5,6, etaA(2,3),'bb -> tt~'
      write(13,108) 5,22,etaA(2,4),'bb -> gaga'
      write(13,108) 5,24,etaA(2,5),'bb -> WW'
      write(13,108) 5,23,etaA(2,6),'bb -> ZZ'
      write(13,108) 5,21,etaA(2,7),'bb -> gg'
      write(13,108) 5,25,etaA(2,8),'bb -> hh'

      write(13,103) 'Block BRANCHING # Branching ratios'
      write(13,102) 15,BRA(1),'BR to tau^+tau^-'
      write(13,102) 5,BRA(2),'BR to bb~'
      write(13,102) 6,BRA(3),'BR to tt~'
      write(13,102) 22,BRA(4),'BR to gaga'
      write(13,102) 24,BRA(5),'BR to WW'
      write(13,102) 23,BRA(6),'BR to ZZ'
      write(13,102) 21,BRA(7),'BR to gg'
      write(13,102) 25,BRA(8),'BR to hh'
      end if
     ...
     &           '(+/- integ.err.)'
         ...
         if (norderggh.eq.0) then
            write(13,102) 1,xsecA(0,1),"LO"
         elseif (norderggh.ge.1) then
            write(13,102) 1,xsecA(0,1),"LO w/ NLO PDFs"
            write(13,102) 2,xsecA(0,2),"NLO"
            write(13,102) 21,ggchanA(0),"NLO gg"
            write(13,102) 22,qgchanA(0),"NLO qg"
            write(13,102) 23,qqchanA(0),"NLO qq"
            write(13,102) 101,xsecerrA(0,1),"+/-: LO w/ NLO PDFs"
            write(13,102) 102,xsecerrA(0,2),"+/-: NLO"
            write(13,102) 121,ggchanerrA(0),"+/-: NLO gg"
            write(13,102) 122,qgchanerrA(0),"+/-: NLO qg"
            write(13,102) 123,qqchanerrA(0),"+/-: NLO qq"
         endif

         else !in case of CP violation print both CP even and odd component

         if ((dist.eq.0).or.(dist.eq.2)) then
          write(13,103) 
     &           "Block XSGGHe # ggh cross sec. in pb (w/o EW)"
         else
            write(13,103)
     &           "Block XSGGHe # ggh cross sec. in pb/GeV  (w/o EW)"
         endif
         if (norderggh.eq.0) then
            write(13,102) 1,xsecA(1,1),"LO CP-even part"
         elseif (norderggh.ge.1) then
            write(13,102) 1,xsecA(1,1),"LO w/ NLO PDFs CP-even part"
            write(13,102) 2,xsecA(1,2),"NLO CP-even part"
            write(13,102) 21,ggchanA(1),"NLO gg CP-even part"
            write(13,102) 22,qgchanA(1),"NLO qg CP-even part"
            write(13,102) 23,qqchanA(1),"NLO qq CP-even part"
         endif

         if ((dist.eq.0).or.(dist.eq.2)) then
          write(13,103) 
     &           "Block XSGGHo # ggh cross sec. in pb (w/o EW)"
         else
            write(13,103)
     &           "Block XSGGHo # ggh cross sec. in pb/GeV  (w/o EW)"
         endif
         if (norderggh.eq.0) then
            write(13,102) 1,xsecA(2,1),"LO CP-odd part"
         elseif (norderggh.ge.1) then
            write(13,102) 1,xsecA(2,1),"LO w/ NLO PDFs CP-odd part"
            write(13,102) 2,xsecA(2,2),"NLO CP-odd part"
            write(13,102) 21,ggchanA(2),"NLO gg CP-odd part"
            write(13,102) 22,qgchanA(2),"NLO qg CP-odd part"
            write(13,102) 23,qqchanA(2),"NLO qq CP-odd part"
         endif

         endif !end of the CP violation specific output

         if (model.le.9) then
         write(13,103) 'Block XSGGHEFF # ggh xsec in heavy top limit '
     &        //'(from ggh@nnlo) (+/- integ.err.)'
         else
         write(13,103) 'Block XSGGHEFFe # ggh xsec in heavy top limit '
     &        //'(from ggh@nnlo) (+/- integ.err.)'
         end if
         if (ltotinc) then
            if (norderggh.ge.1) then
               write(13,102) 1,SUgghA(1,2,2,0),"NLO"
               write(13,102) 101,SUggherrA(1,2,2,0),"+/-: NLO"
               write(13,102) 19,SUgghA(1,norderggh+1,2,0),"NLO'"
               write(13,102) 191,SUggherrA(1,norderggh+1,2,0),
     &              "+/-: NLO'"
               if (nsubprocggh.eq.10) then
!     ggh@nnlo computed all subchannels separately:
                  write(13,102) 10,SUgghA(1,2,2,-1),"NLO: soft+virt"
                  write(13,102) 11,SUgghA(1,2,2,1),"NLO: gg-channel"
                  write(13,102) 12,SUgghA(1,2,2,2),"NLO: qg-channel"
                  write(13,102) 13,SUgghA(1,2,2,3),"NLO: qqb-channel"
                  write(13,102) 110,SUggherrA(1,2,2,-1)
     &                 ,"+/- : NLO soft+virt"
                  write(13,102) 111,SUggherrA(1,2,2,1),"+/- : NLO gg"
                  write(13,102) 112,SUggherrA(1,2,2,2),"+/- : NLO qg"
                  write(13,102) 113,SUggherrA(1,2,2,3),"+/- : NLO qqb"
               endif
            endif
            do i=3,4
               if (norderggh.ge.(i-1)) then
                tmpstring = orderstring(i)
                write(13,102) i-1,SUgghA(1,i,i,0),tmpstring
                write(13,102) 100+i-1,SUggherrA(1,i,i,0),"+/-: "
     &//tmpstring
                if (nsubprocggh.eq.10) then
!     ggh@nnlo computed all subchannels separately:
                   write(13,102) 10*(i-1),SUgghA(1,i,i,-1),tmpstring/
     &                    /": soft+virt"
                   write(13,102) 10*(i-1)+1,SUgghA(1,i,i,1),tmpstring/
     &                    /": gg-channel"
                   write(13,102) 10*(i-1)+2,SUgghA(1,i,i,2),tmpstring/
     &                    /": qg-channel"
                   write(13,102) 10*(i-1)+3,SUgghA(1,i,i,3),tmpstring/
     &                    /": qqb-channel"
                   write(13,102) 10*(i-1)+4,SUgghA(1,i,i,4),tmpstring/
     &                    /": qq-channel"
                   write(13,102) 10*(i-1)+5,SUgghA(1,i,i,5),tmpstring/
     &                    /": qu-channel"
                   write(13,102) 100+10*(i-1),SUggherrA(1,i,i,-1)
     &                    ,"+/- : soft+virt"
                   write(13,102) 100+10*(i-1)+1,SUggherrA(1,i,i,1),
     &                    "+/- : "//tmpstring//" gg"
                   write(13,102) 100+10*(i-1)+2,SUggherrA(1,i,i,2),
     &                    "+/- : "//tmpstring//" qg"
                   write(13,102) 100+10*(i-1)+3,SUggherrA(1,i,i,3),
     &                    "+/- : "//tmpstring//" qqb"
                   write(13,102) 100+10*(i-1)+4,SUggherrA(1,i,i,4),
     &                    "+/- : "//tmpstring//" qq"
                   write(13,102) 100+10*(i-1)+5,SUggherrA(1,i,i,5),
     &                    "+/- : "//tmpstring//" qu"
                  endif
               endif
            enddo
         ...
         if (model.le.9) then
         write(13,102) 4,elwA(1),"electroweak factor"
         else !output in case of CP violation CMSSM - detailed subchannel output not available
         write(13,103) 'Block XSGGHEFFo # ggh xsec in heavy top limit '
     &        //'(from ggh@nnlo) (+/- integ.err.)'
         write(13,102) 4,elwA(1),"CP-even electroweak factor"
         if (ltotinc) then
            if (norderggh.ge.1) then
               write(13,102) 1,SUgghA(2,2,2,0),"NLO CP-odd"
               write(13,102) 101,SUggherrA(2,2,2,0),"+/-: NLO CP-odd"
               write(13,102) 19,SUgghA(2,3,2,0),"NLO' CP-odd"
               write(13,102) 191,SUggherrA(2,3,2,0),
     &              "+/-: NLO' CP-odd"
            end if
            do i=3,4
               if (norderggh.ge.(i-1)) then
                tmpstring = orderstring(i)
                write(13,102) i-1,SUgghA(2,i,i,0),tmpstring//" CP-odd"
                write(13,102) 100+i-1,SUggherrA(2,i,i,0),"+/-: "
     &//tmpstring//" CP-odd"
               endif
            enddo
         end if
     ...
     &           /' xsec in pb/GeV (+/- integ.err.)'
            ...
            if (lqqhkey) write(13,103) '# calculated '//ptname(ptn1)
     &           //ptname(ptn2)//'->H xsec - see Block QQH below'
            write(13,102) 1,SUbbhA(0,1),"LO"
            write(13,102) 101,SUbbherrA(0,1),"+/-: LO"
            if (norderbbh.gt.0) then
               write(13,102) 2,SUbbhA(0,2),"NLO"
               write(13,102) 102,SUbbherrA(0,2),"+/-: NLO"
            end if
            if (norderbbh.gt.1) then
               if ((ptc.eq.0).and.(rapc.eq.0).and.(dist.eq.0)) then
                  write(13,102) 3,SUbbhA(0,3),"NNLO"
                  write(13,102) 103,SUbbherrA(0,3),"+/-: NNLO"
               end if
            end if
         endif
      endif
      ...
      if (norderbbh.ge.0) then
         if ((dist.eq.0).or.(dist.eq.1)) then
            if (dist.eq.0) then
            write(13,103) 'Block XSBBHe # '//qqhstring/
     &           /' xsec in pb (+/- integ.err.)'
            else
            write(13,103) 'Block XSBBHe # '//qqhstring/
     &           /' xsec in pb/GeV (+/- integ.err.)'
            ...
            if (lqqhkey) write(13,103) '# calculated '//ptname(ptn1)
     &           //ptname(ptn2)//'->H xsec - see Block QQH below'
            write(13,102) 1,SUbbhA(1,1),"LO CP-even"
            write(13,102) 101,SUbbherrA(1,1),"+/-: LO"
            if (norderbbh.gt.0) then
               write(13,102) 2,SUbbhA(1,2),"NLO CP-even"
               write(13,102) 102,SUbbherrA(1,2),"+/-: NLO"
            end if
            if (norderbbh.gt.1) then
               if ((ptc.eq.0).and.(rapc.eq.0).and.(dist.eq.0)) then
                  write(13,102) 3,SUbbhA(1,3),"NNLO CP-even"
                  write(13,102) 103,SUbbherrA(1,3),"+/-: NNLO"
               end if
            end if
         endif
      endif
      if (norderbbh.ge.0) then
         if ((dist.eq.0).or.(dist.eq.1)) then
            if (dist.eq.0) then
            write(13,103) 'Block XSBBHo # '//qqhstring/
     &           /' xsec in pb (+/- integ.err.)'
            else
            write(13,103) 'Block XSBBHo # '//qqhstring/
     &           /' xsec in pb/GeV (+/- integ.err.)'
            ...
            if (lqqhkey) write(13,103) '# calculated '//ptname(ptn1)
     &           //ptname(ptn2)//'->H xsec - see Block QQH below'
            write(13,102) 1,SUbbhA(2,1),"LO CP-odd"
            write(13,102) 101,SUbbherrA(2,1),"+/-: LO"
            if (norderbbh.gt.0) then
               write(13,102) 2,SUbbhA(2,2),"NLO CP-odd"
               write(13,102) 102,SUbbherrA(2,2),"+/-: NLO"
            end if
            if (norderbbh.gt.1) then
               if ((ptc.eq.0).and.(rapc.eq.0).and.(dist.eq.0)) then
                  write(13,102) 3,SUbbhA(2,3),"NNLO CP-odd"
                  write(13,102) 103,SUbbherrA(2,3),"+/-: NNLO"
               end if
            end if
         endif
      endif
      ...
      call writecouplingsa(gb13A,gbh123A,gbh213A,gb23A,gt13A,
     & gth123A,gth213A,gt23A,gbA,gcA,gtA)
...
c.. Henry's edits; prompt python to stop looking for output
      write(13, *) '**output file ends here'
...
C-{{{ subroutine writecouplings
      ...
      !This routine is only for compatibility with external codes, that
      !called writecouplings already previously.
      implicit none
      ...
      include '../commons/common-keys.f'
       ...
       gb13A(1) = gb13
       gbh123A(1) = dcmplx(0.d0,0.d0) !zero in the real MSSM
       gbh213A(1) = dcmplx(0.d0,0.d0)
       gb23A(1) = gb23
       gt13A(1) = gt13
       gth123A(1) = dcmplx(0.d0,0.d0)
       gth213A(1) = dcmplx(0.d0,0.d0)
       gt23A(1) = gt23
       gbA(1) = gb
       gcA(1) = gc
       gtA(1) = gt
...
C-{{{ subroutine writecouplings
      ...
      subroutine writecouplingsa(gb13A,gbh123A,gbh213A,gb23A,gt13A,
     & gth123A,gth213A,gt23A,gbA,gcA,gtA)
      implicit none
      double precision vev,vevS,Sqrt2
      character psstr*7
      integer Sind2, count1, count2

      double complex gb13A(2), gbh123A(2), gbh213A(2), gb23A(2)
      double complex gt13A(2), gth123A(2), gth213A(2), gt23A(2)
      double complex gbA(2), gcA(2), gtA(2)
      double complex D1, D2, D1C, D2C
      double precision sbeta, cbeta
...
!     effective couplings in (N)MSSM
      if ((model.ge.1).and.(model.le.9)) then
         write(13,1030) 
     &        "Block HGGSUSY # couplings of ",Sind2,". "//psstr/
     &        /" H to 3. gen."
         write(13,102) 101,Real(gtA(1)),'Re(g_t^phi)'
         write(13,102) 1010,Aimag(gtA(1)),'Im(g_t^phi)'
         if ((model.eq.1).or.(model.eq.3)) then
            write(13,102) 111,Real(gt13A(1)),'Re(g_st11^phi)'
            write(13,102) 122,Real(gt23A(1)),'Re(g_st22^phi)'
            write(13,102) 112,Real(gth123A(1)),'Re(g_st12^phi)'
            write(13,102) 121,Real(gth213A(1)),'Re(g_st21^phi)'
         end if
         write(13,102) 201,Real(gbA(1)),'Re(g_b^phi)'
         write(13,102) 2010,Aimag(gbA(1)),'Im(g_b^phi)'
         if ((model.eq.1).or.(model.eq.3)) then
            write(13,102) 211,Real(gb13A(1)),'Re(g_sb11^phi)'
            write(13,102) 222,Real(gb23A(1)),'Re(g_sb22^phi)'
            write(13,102) 212,Real(gbh123A(1)),'Re(g_sb12^phi)'
            write(13,102) 221,Real(gbh213A(1)),'Re(g_sb21^phi)'
         end if
     ...
     &           /" H to 4. gen."
         ...
         if ((model.eq.1).or.(model.eq.3)) then !MSSM and NMSSM - based on effective mixing angle
            write(13,103)
     &         'Block BBHREWEIGHT # top and bottom reweighting'
     &        //'factors (effective mixing angle)'
         if (pseudo.eq.1) then
            write(13,102) 1,Amx(Sind,2)/tanb,'g_t'
         else if (pseudo.eq.0) then
            write(13,102) 1,Hmx(Sind,2)/dsin(datan(tanb)),'g_t'
         end if
         if (pseudo.eq.1) then
            write(13,102) 2,Amx(Sind,2)*tanb* (1.d0 - (1.d0/tanb
     &           **2 - Amx(Sind,3)*vev/(Amx(Sind,2)*vevS*tanb))*delmb)
     &           * (1.d0/(1.d0+delmb))
     &           ,'g_b incl. 1/(1+Delta_b)*(1+C*Delta_b)'
         else if (pseudo.eq.0) then
            write(13,102) 2,Hmx(Sind,1)/dcos(datan(tanb))* (1.d0
     &             + (Hmx(Sind,2)/(tanb*Hmx(Sind,1))
     &             + Hmx(Sind,3)*vev*dcos(datan(tanb))
     ...
     &"Block BBHREWEIGHT # top and bottom reweighting factors"
         write(13,102) 1,Real(gtA(1)),'g_t'
         write(13,102) 2,Real(gbA(1)),'g_b'
         end if
      endif

!     effective couplings in CMSSM
      if (model.eq.10) then
         write(13,1030) 
     &      "Block HGGSUSYe # CP-even couplings of ",Sind2,". "//psstr/
     &        /" H to 3. gen."
         write(13,102) 101,Real(gtA(1)),'Re(g_t^phi)'
         write(13,102) 1010,Aimag(gtA(1)),'Im(g_t^phi)'
         write(13,102) 111,Real(gt13A(1)),'Re(g_st11^phi)'
         ...
         write(13,102) 112,Real(gth123A(1)),'Re(g_st12^phi)'
         write(13,102) 121,Real(gth213A(1)),'Re(g_st21^phi)'
         write(13,102) 1110,Aimag(gt13A(1)),'Im(g_st11^phi)'
         write(13,102) 1220,Aimag(gt23A(1)),'Im(g_st22^phi)'
         write(13,102) 1120,Aimag(gth123A(1)),'Im(g_st12^phi)'
         write(13,102) 1210,Aimag(gth213A(1)),'Im(g_st21^phi)'
         write(13,102) 201,Real(gbA(1)),'Re(g_b^phi)'
         write(13,102) 2010,Aimag(gbA(1)),'Im(g_b^phi)'
         write(13,102) 211,Real(gb13A(1)),'Re(g_sb11^phi)'
         write(13,102) 222,Real(gb23A(1)),'Re(g_sb22^phi)'
         write(13,102) 212,Real(gbh123A(1)),'Re(g_sb12^phi)'
         write(13,102) 221,Real(gbh213A(1)),'Re(g_sb21^phi)'
         write(13,102) 2110,Aimag(gb13A(1)),'Im(g_sb11^phi)'
         write(13,102) 2220,Aimag(gb23A(1)),'Im(g_sb22^phi)'
         write(13,102) 2120,Aimag(gbh123A(1)),'Im(g_sb12^phi)'
         write(13,102) 2210,Aimag(gbh213A(1)),'Im(g_sb21^phi)'
         write(13,1030) 
     &      "Block HGGSUSYo # CP-odd couplings of ",Sind2,". "//psstr/
     &        /" H to 3. gen."
         write(13,102) 101,Real(gtA(2)),'Re(g_t^phi)'
         write(13,102) 1010,Aimag(gtA(2)),'Im(g_t^phi)'
         write(13,102) 111,Real(gt13A(2)),'Re(g_st11^phi)'
         write(13,102) 122,Real(gt23A(2)),'Re(g_st22^phi)'
         write(13,102) 112,Real(gth123A(2)),'Re(g_st12^phi)'
         write(13,102) 121,Real(gth213A(2)),'Re(g_st21^phi)'
         write(13,102) 1110,Aimag(gt13A(2)),'Im(g_st11^phi)'
         write(13,102) 1220,Aimag(gt23A(2)),'Im(g_st22^phi)'
         write(13,102) 1120,Aimag(gth123A(2)),'Im(g_st12^phi)'
         write(13,102) 1210,Aimag(gth213A(2)),'Im(g_st21^phi)'
         write(13,102) 201,Real(gbA(2)),'Re(g_b^phi)'
         write(13,102) 2010,Aimag(gbA(2)),'Im(g_b^phi)'
         write(13,102) 211,Real(gb13A(2)),'Re(g_sb11^phi)'
         write(13,102) 222,Real(gb23A(2)),'Re(g_sb22^phi)'
         write(13,102) 212,Real(gbh123A(2)),'Re(g_sb12^phi)'
         write(13,102) 221,Real(gbh213A(2)),'Re(g_sb21^phi)'
         write(13,102) 2110,Aimag(gb13A(2)),'Im(g_sb11^phi)'
         write(13,102) 2220,Aimag(gb23A(2)),'Im(g_sb22^phi)'
         write(13,102) 2120,Aimag(gbh123A(2)),'Im(g_sb12^phi)'
         write(13,102) 2210,Aimag(gbh213A(2)),'Im(g_sb21^phi)'
         cbeta=dcos(datan(tanb))
         sbeta=dsin(datan(tanb))
         D1 = CHmx(Sind,1)/cbeta + CHmx(Sind,2)/sbeta*cdelmb
         D1 = D1 / (1.d0 + cdelmb)
         D1C = CHmx(Sind,1)/cbeta + CHmx(Sind,2)/sbeta*Conjg(cdelmb)
         D1C = D1C / (1.d0 + Conjg(cdelmb))
         D2 = CHmx(Sind,3)*(tanb - cdelmb/tanb)
         D2 = D2 / (1.d0 + cdelmb)
         D2C = CHmx(Sind,3)*(tanb - Conjg(cdelmb)/tanb)
         D2C = D2C / (1.d0 + Conjg(cdelmb))
         write(13,103) 
     &"Block BBHREWEIGHT # top and bottom reweighting factors"
         write(13,102) 10,Real(CHmx(Sind,2)/sbeta),'Real g_t even'
         write(13,102) 11,Aimag(CHmx(Sind,2)/sbeta),'Imag g_t even'
         write(13,102) 12,Real(CHmx(Sind,3)*1.d0/tanb),'Real g_t odd'
         write(13,102) 13,Aimag(CHmx(Sind,3)*1.d0/tanb),'Imag g_t odd'
         write(13,102) 20,Real(1/2.d0*(D1 + D1C
     &   + DCMPLX(0.d0,1.d0)* (D2 - D2C))),'Real g_b even'
         write(13,102) 21,Aimag(1/2.d0*(D1 + D1C
     &   + DCMPLX(0.d0,1.d0)* (D2 - D2C))),'Imag g_b even'
         write(13,102) 22,Real(1/2.d0*(D2 + D2C
     &   - DCMPLX(0.d0,1.d0)* (D1 - D1C))),'Real g_b odd'
         write(13,102) 23,Aimag(1/2.d0*(D2 + D2C
     &   - DCMPLX(0.d0,1.d0)* (D1 - D1C))),'Imag g_b odd'
      endif
...
!additional output
     ...
     &       Sind2,'. '//psstr//' Higgs mass in GeV'
      ...
      if ((model.eq.2).and.(thdmcflag).and.(.not.(thdmccmdflag))) then
     ...
     &        ,'unitarity, 0=failed, 1=ok'
      ...
      if ((model.eq.1).and.(fhflag)) then
         write(13,103) 'Block ALPHA # eff. Higgs mixing angle from FH'
         write(13,107) alpha,'alpha'
      if ((ZHiggsapply.eq.1).and.(pseudo.eq.0)) then
         write(13,103) 'Block REALHMNMIX # Z matrix'
     &        //' from FH*tree-level alpha'
         do count1=1,2
            do count2=1,2
               write(13,108) count1,count2,Real(CHmx(count1,count2)),
     &              'Mixing matrix'
            end do
         end do
         write(13,103) 'Block COMPHMNMIX # Z matrix'
     &        //' from FH*tree-level alpha'
         do count1=1,2
            do count2=1,2
               write(13,108) count1,count2,Aimag(CHmx(count1,count2)),
     &              'Mixing matrix'
            end do
         end do
      end if
      end if
      if ((model.eq.10).and.(fhflag)) then
         write(13,103) 'Block REALHMNMIX # Z matrix'
     &        //' from FH*tree-level alpha'
         do count1=1,3
            do count2=1,3
     ...
     &        ,'used Ab in GeV - def. accord. to scheme'
      ...
      if (model.eq.10) then
         write(13,103) 'Block STOPMIXOUT # stop mixing matrix'
         write(13,108) 1,1,Real(RU11),'V_11'
         write(13,108) 1,2,Real(RU12),'V_12'
         write(13,108) 2,1,Real(RU21),'V_21'
         write(13,108) 2,2,Real(RU22),'V_22'
         write(13,103) 'Block STOPMIXOUTIM # stop mixing matrix'
         write(13,108) 1,1,Aimag(RU11),'V_11'
         write(13,108) 1,2,Aimag(RU12),'V_12'
         write(13,108) 2,1,Aimag(RU21),'V_21'
         write(13,108) 2,2,Aimag(RU22),'V_22'
         write(13,103) 'Block SBOTMIXOUT # sbottom mixing matrix'
         write(13,108) 1,1,Real(RD11),'V_11'
         write(13,108) 1,2,Real(RD12),'V_12'
         write(13,108) 2,1,Real(RD21),'V_21'
         write(13,108) 2,2,Real(RD22),'V_22'
         write(13,103) 'Block SBOTMIXOUTIM # sbottom mixing matrix'
         write(13,108) 1,1,Aimag(RD11),'V_11'
         write(13,108) 1,2,Aimag(RD12),'V_12'
         write(13,108) 2,1,Aimag(RD21),'V_21'
         write(13,108) 2,2,Aimag(RD22),'V_22'
      endif
     ...
     &           ,'m_b for bottom Yukawa'
      ...
      if ((model.eq.1).or.(model.eq.10)) then
...
!     Reconstruction of the input file
     ...
     &     'Chosen model: 0=SM, 1=MSSM, 2=2HDM, 3=NMSSM'
      ...
      htypestring(10) = '11=H1,12=H2,13=H3'
     ...
     &     ,'ggh@nnlo subprocesses: 0=all, 10=ind. contributions'
      ...
      if (thdmcflag) then
         ...
         if (thdmccmdflag) write(file,1020) -1,1,'cmd mode'
     ...
     &     (model.eq.1).or.(model.eq.3).or.(model.eq.10)) then
...
!Specific SUSY parameters
      if (model.eq.1) then
         ...
         if ((fhflag).and.(MHp.lt.0)) then
         write(file,102) 26,mA0,'mA'
         else if ((fhflag).and.(MA0.lt.0)) then
         write(file,102) 27,mHp,'mHp'
         end if
         ...
         if (fhflag) then
            ...
            write(file,1020) 11,ZHiggsapply,
     &             'Z Higgs rather than eff. mixing angle'
            write(file,103) 'Block FEYNHIGGS'
            !to be included if changed to FH output
            !if (fhcmdflag) write(file,102) -1,1,'cmd mode'
...
!CMSSM specific parameters
      if (model.eq.10) then
         write(file,103) 'Block EXTPAR'
         write(file,102) 3, Real(CMgl),'Gluino mass'
         write(file,102) 11,Real(cAt),'A_t'
         write(file,102) 12,Real(cAb),'A_b'
         write(file,102) 23,Real(CmuSUSY),'mu'
         if (fhflag) write(file,102) 27,MHp,'MHp'
         if (.not.onshellflag) then
            write(file,102) 43,M3SQ,'M3SQ'
            write(file,102) 46,M3SU,'M3SU'
            write(file,102) 49,M3SD,'M3SD'
         end if
         write(file,103) 'Block IMEXTPAR'
         write(file,102) 3,aimag(CMgl),'Gluino mass'
         write(file,102) 11,aimag(cAt),'A_t'
         write(file,102) 12,aimag(cAb),'A_b'
         write(file,102) 23,aimag(CmuSUSY),'mu'
         if (fhflag) then
            write(file,103) 'Block FEYNHIGGSFLAGS'
            write(file,1020) 1,mssmpart,'mssmpart'
            write(file,1020) 2,fieldren,'fieldren'
...
C-{{{ subroutine screenoutputsushi
      ...
      subroutine screenoutputsushi(ggchanA,qgchanA,qqchanA,
     & ggchanerrA,qgchanerrA,qqchanerrA,
     & SUbbhA,SUbbherrA,
     & SUgghA,SUggherrA,
     & xsecA,xsecerrA,xeffA,xefferrA,
...
!norder,mw,mc,ew,mz,mbmb,renscheme,mt,pseudo,tanbresum,pseudorap
     ...
     &"Note: NNLO SUSY contribution not taken into account."
               write(*,*) 
     &"Use order ggh = 12 for approximate NNLO stop cont. for h."
               write(*,105)
            else
               write(*,105)
               write(*,*) 
     ...
     &"Thus NNLO results will not be given."
      ...
      if (norderbbh.ge.0) then
         if ((dist.eq.0).or.(dist.eq.1)) then
            ...
            write(*,106) "LO: ",SUbbhA(0,1),SUbbherrA(0,1),hepunit
            if (norderbbh.gt.0) then
               write(*,106) "NLO: ",SUbbhA(0,2),SUbbherrA(0,2),hepunit
            end if
            if (norderbbh.gt.1) then
            if ((.not.((ptc.gt.0).or.(rapc.gt.0))).and.(dist.eq.0)) then
               write(*,106) "NNLO: ",SUbbhA(0,3),SUbbherrA(0,3),hepunit
            end if
            end if
         end if
      end if
      ...
      if (norderggh.ge.0) then
         ...
         write(*,106) "LO: ",xsecA(0,1),xsecerrA(0,1),hepunit
         if (norderggh.gt.0) then
            write(*,106) "gg: ",ggchanA(0),ggchanerrA(0),hepunit
            write(*,106) "qg: ",qgchanA(0),qgchanerrA(0),hepunit
            write(*,106) "qq: ",qqchanA(0),qqchanerrA(0),hepunit
            write(*,106) "NLO: ",xsecA(0,2),xsecerrA(0,2),hepunit
!            if ((dist.eq.0).and.((ew.ge.1).or.(norderggh.ge.2))) then
     ...
     & then
         write(*,106) "xsec: ",xeffA(0),xefferrA(0),hepunit
!               write(*,107)
!     &" Details with respect to the weighted result in output-file."
         ...
         if (muranalytic) then
         write(*,109)
     &   "  muR unc.: ",dabs(gghXSvarA(0,1)),gghXSvarA(0,2),hepunit
         write(*,107)
     &   " The scale uncertainty is obtained by a variation"
         write(*,1030) " of muR within [",dexp(xeffvarA(0,1))/murggh,
     &  ',',dexp(xeffvarA(0,2))/murggh,']*',murggh,' GeV.'
         end if
      end if
...
C-{{{ subroutine printcopysushi:
     ...
     & '  #'
      ...
      write(iu,1) "#                        (stefan.liebler@kit.edu)  #"
      write(iu,1) "#            BU Wuppertal, RWTH Aachen, DESY, KIT  #"
      write(iu,1) "# Contributions by:                                #"
      write(iu,1) "# Shruti Patel (KIT): SusHiMi for CP violation     #"
      write(iu,1) "# Lars-Thorben Moos (RWTH): Link to ext. codes     #"
      write(iu,1) "#--------------------------------------------------#"
