...
def truth_vertices(eventWise, jet_name, batch_length=np.inf):
    """
    Identify the index of the secondary vertex.
    This is the point that the tag particle decays.
    Results are appended to the EventWise file.
    ...
    # retreve number of events and the existing data
...
def closest_approches(start_points, direction_vectors):
    ...
    to every other line.
    Note that this cannot be done in 4-vectors unless all lines can be
    guaranteed not to come withing a time-like distance of each other.
...
def find_vertices(eventWise, jet_name, vertex_name, batch_length=np.inf, threshold=0.02):
    """
    Make an analytic estimate of the location of the vertices
    from the tracks in the jet.
    Results are appended to the eventwise.
